...
### 2. What do you mean by @Transactional Annotation
**Ans:** We can make Class as Transactional and a method as Transactional 
1. when we make a class as Transactional it makes all the public method as Transactional
...
1. **Serial GC**<br> Serial GC only one thread do the memory clean-up and as we know GC is very expensive task so Seral GC is not a feasible solution because application has to wait for longer Time 
...
4. **G1 GC** <br> G1 GC is the better version of Concurrent mark and sweep where it ensures that I will not let you system get down
...
### 8. In a Java (or C#) method, if a return statement is executed inside the try block, will finally block still execute?
...
**Happens-Before Relationship:** It prevents the compiler from reordering code around the volatile variable, ensuring that any instructions before to write are completed first.
...
### 10. JVM parameters
1. **Heap Size**: -Xms (initial heap size) and -Xmx (maximum heap size) parameters control the amount of memory allocated to the JVM for object storage. Setting these values appropriately can help optimize performance and prevent OutOfMemoryErrors.
2. **Garbage Collection**: -XX:+UseG1GC (use G1 Garbage Collector) or -XX:+UseConcMarkSweepGC (use Concurrent Mark Sweep Garbage Collector) can be used to specify the garbage collection algorithm. Choosing the right GC can improve application performance and reduce latency.
3. **JIT Compiler**: -XX:+UseJIT (enable Just-In-Time compilation) allows the JVM to compile bytecode into native machine code at runtime, improving performance.
4. **Logging and Debugging**: -Xloggc (log garbage collection events) and -XX:+PrintGCDetails (print detailed GC information) can be used to monitor and analyze the behavior of the JVM, helping to identify performance bottlenecks and memory issues.
